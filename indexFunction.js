  ...
  products.map(async (product) => {
    ...
    let response = await fetch(api, {
      ...
      headers: {
        ...
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI2NWE2OWYwM2RjZjZkNzAwMTgzODZmZDAiLCJpYXQiOjE3MDYwMjA0MDcsImV4cCI6MTcwNzIzMDAwN30.gUJiVnvlVxI4wcr5ejMJRFmtCZLnxS7IsHYDH1dafr0",
      },
    });
...
const getFetch = async () => {
     ...
     <div class="collapse navbar-collapse" id="navbarNavDropdown">
       <ul class="navbar-nav">
         <li class="nav-item dropdown">
           ...
           <ul class="dropdown-menu backoffice">
            <h5> Produtcs: </h5>
           </ul>
         </li>
       </ul>
  ...
  productsApi.map((allProducts) => {
    container.innerHTML += `<div class="card col-4">
      ...
      <h5> ${allProducts.name}, ${allProducts.brand} </h5>
      <p> ${allProducts.description} </p>
      <h4> <button class="add"> ${allProducts.price} </button> <h4>
      </div>
      </div>`;
  });
  } catch (error) {
